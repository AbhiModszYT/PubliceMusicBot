...
async def log_(client, message, _):
    try:
        if await is_heroku():
            ...
            data = HAPP.get_log()
            link = await PubliceMusicBin(data)
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            return await message.reply_text(
                f"{check_var}: <code>{heroku_config[check_var]}</code>"
            )
        else:
...
<u>Usage:</u>
Total Used: <code>{AppHours}</code> h   <code>{AppMinutes}</code>m  [<code>{AppPercentage} </code>%]
...
async def update_(client, message, _):
    ...
    if len(_final_updates_) > 4096:
        ...
        nrs = await response.edit(
            f"<b>A new update is available for the Bot!</b>\n\n➣ Pushing Updates Now</code>\n\n<u>Updates:</u>\n\n<a href={url}>Click Here to checkout Updates </a>)"
        )
    else:
        nrs = await response.edit(
    ...
    if await is_heroku():
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{app.mention} has just restarted herself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
                    )
        ...
        for x in served_chats:
            try:
                await app.send_message(
                    x,
                    f"{app.mention} has just restarted herself. Sorry for the issues.\n\nStart playing after 2-3 min again.",
                )
                await remove_active_chat(x)
                await remove_active_video_chat(x)
            except Exception:
